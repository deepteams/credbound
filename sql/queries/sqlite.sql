...
INSERT INTO credbound_instance (singleton, initialized_at) VALUES (1, ?1);
...
JOIN credbound_user_emails primary_email ON primary_email.user_id = u.id AND primary_email.is_primary = 1
...
WHERE a.role = 'root' AND u.disabled = 0;
...
SELECT id, user_id, address, is_primary, verified_at, verification_digest, verification_expires_at, created_at, updated_at
FROM credbound_user_emails WHERE address = ?1;
...
UPDATE credbound_user_emails SET is_primary = 0, updated_at = ?2 WHERE user_id = ?1 AND is_primary = 1;

-- name: SetPrimaryEmail :execrows
UPDATE credbound_user_emails SET is_primary = 1, updated_at = ?3
WHERE id = ?2 AND user_id = ?1 AND verified_at IS NOT NULL;

-- name: DeleteEmail :execrows
DELETE FROM credbound_user_emails WHERE id = ?2 AND user_id = ?1 AND is_primary = 0;
...
UPDATE credbound_password_credentials SET hash = ?2, updated_at = ?3 WHERE user_id = ?1 AND hash = @previous_hash;
...
WHERE m.workspace_id = ?1 AND m.role = 'admin' AND m.status = 'active' AND u.disabled = 0;
...
VALUES (?1, ?2, 0, 0, ?3, ?4)
ON CONFLICT (user_id) DO UPDATE SET encrypted_secret = excluded.encrypted_secret, active = 0, last_used_step = 0, updated_at = excluded.updated_at
WHERE credbound_totp_factors.active = 0;
...
UPDATE credbound_totp_factors SET encrypted_secret = ?2, active = 1, last_used_step = ?3, updated_at = ?4 WHERE user_id = ?1 AND active = 0;

-- name: UseTOTP :execrows
UPDATE credbound_totp_factors SET last_used_step = ?2, updated_at = ?3 WHERE user_id = ?1 AND active = 1 AND last_used_step < ?2;
...
INSERT INTO credbound_sessions (id, user_id, method, level, authenticated_at, second_factor_required, user_agent, ip_address, digest, created_at, last_seen_at, expires_at, revoked_at)
...
UPDATE credbound_sessions SET revoked_at = ?2 WHERE user_id = ?1 AND revoked_at IS NULL;
...
SELECT DISTINCT grant_id FROM credbound_oauth_refresh_tokens WHERE family_id = ?1;
...
DELETE FROM credbound_workspace_domains WHERE domain = ?1 AND confirmed_at IS NULL AND created_at < @stale_before;
