	...
	return s.oauthMutate(ctx, commit, func(_ *sql.Tx, q *db.Queries) error {
		...
		if initialID != "" {
			...
			count, err := q.OAuthUseInitialAccessToken(ctx, db.OAuthUseInitialAccessTokenParams{
				ID: token.ID, RegistrationCount: int64(token.RegistrationCount), DataJson: oauthParam(token), RegistrationCount_2: int64(previousCount), ExpiresAt: usedAt,
			})
			if err != nil || count != 1 {
				return credbound.ErrConflict
			}
...
WHERE issuer_id = ? AND (? = '' OR created_at < ? OR (created_at = ? AND id < ?))
...
func (s *Store) CreateOAuthInitialAccessToken(ctx context.Context, value credbound.OAuthInitialAccessToken, commit credbound.Commit) error {
	return s.oauthMutate(ctx, commit, func(_ *sql.Tx, q *db.Queries) error {
		return mapError(q.OAuthInsertInitialAccessToken(ctx, db.OAuthInsertInitialAccessTokenParams{
			ID: value.ID, IssuerID: value.IssuerID, Prefix: value.Prefix, RegistrationCount: int64(value.RegistrationCount), MaxRegistrations: int64(value.MaxRegistrations), ExpiresAt: value.ExpiresAt, RevokedAt: nullableTime(value.RevokedAt), DataJson: oauthParam(value),
		}))
	})
}
...
WHERE (? = '' OR user_id = ?) AND (? = '' OR workspace_id = ?)
AND (? = '' OR created_at < ? OR (created_at = ? AND id < ?))
ORDER BY created_at DESC, id DESC LIMIT ?`, []any{userID, userID, workspaceID, workspaceID}, page, func(value credbound.OAuthGrant) credbound.OAuthGrant { return value })
}
...
func oauthPage[T any](s *Store, ctx context.Context, query string, filterArgs []any, page credbound.PageRequest, public func(T) T) iter.Seq2[credbound.PageEvent[T], error] {
	return func(yield func(credbound.PageEvent[T], error) bool) {
		...
		args = append(args, cursor.ID, cursor.Time, cursor.Time, cursor.ID, page.Limit+1)
