...
func (q *Queries) CountUserAuthenticationMethods(ctx context.Context, userID string) (int64, error) {
...
DELETE FROM credbound_workspace_domains WHERE domain = ?1 AND confirmed_at IS NULL AND created_at < ?2
`
...
UPDATE credbound_password_credentials SET hash = ?2, updated_at = ?3 WHERE user_id = ?1 AND hash = ?4
`
